...
- **MUST:** Use `x.x.x` instead of guessing the release a change ships in: `@since x.x.x`, `@deprecated x.x.x` (a space, not a tab), and `'x.x.x'` as the version argument of deprecation functions (`_deprecated_function()`, `wc_deprecated_function()`, `apply_filters_deprecated()`, ...). woorelease replaces them with the real version at release, in the paths listed under `config.version_replace_paths` in `package.json` (quoted strings because `config.version_replace_strings` is on). An unquoted `x.x.x`, or one in JS, is not replaced. The `Version placeholders` lint job flags placeholders that won't be replaced and guessed unreleased versions.
...
Update routines run on upgrade, keyed off the stored `wc_stripe_version` option; one-off migration classes live in `includes/migrations/`. Two invariants:

- Migrations are one-shot: they are keyed so a site that has updated past them never re-runs them. A migration added after a version has shipped needs a new key — never reuse or edit an already-shipped one.
