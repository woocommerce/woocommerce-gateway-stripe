...
| PHPUnit (parallel) | `npm run test:php:parallel` | Runs tests in parallel via paratest; requires the legacy Docker environment. Set `XDEBUG_MODE_PHPUNIT=coverage` to enable coverage. |
