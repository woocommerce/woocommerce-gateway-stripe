...
class WC_Stripe_Ability_Base_Test extends WP_UnitTestCase {
	...
	public function test_retrieve_returns_wp_error_when_stripe_api_returns_null() {
		...
		$mock_unauthorized_filter = function () {
			return [
				'response' => [
					'code'    => 401,
	...
	public function test_retrieve_surfaces_stripe_api_error_payload() {
		...
		$mock_payment_required_filter = function () {
			return [
				'response' => [
					'code'    => 402,
		...
		add_filter( 'pre_http_request', $mock_payment_required_filter );
		...
		try {
			...
			remove_filter( 'pre_http_request', $mock_payment_required_filter );
		}
	}
	...
	public function test_retrieve_returns_array_on_success() {
		...
		$mock_successful_response_filter = function () {
			return [
				'response' => [
					'code'    => 200,
					'message' => 'OK',
		...
		add_filter( 'pre_http_request', $mock_successful_response_filter );
