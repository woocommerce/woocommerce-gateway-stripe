...
class WC_Stripe_Amazon_Pay_Controller_Test extends WP_UnitTestCase {
	...
	public function test_admin_scripts(): void {
		...
		$this->assertTrue( wp_script_is( 'wc-stripe-amazon-pay-settings', 'registered' ) );
		$this->assertTrue( wp_script_is( 'wc-stripe-amazon-pay-settings', 'enqueued' ) );
		$this->assertTrue( wp_style_is( 'wc-stripe-amazon-pay-settings', 'registered' ) );
		$this->assertTrue( wp_style_is( 'wc-stripe-amazon-pay-settings', 'enqueued' ) );
	}

	/**
	 * The Amazon Pay simulator additionally needs the account-country gate; assert all gates ship.
	 *
	 * @return void
	 */
