...
class WC_Stripe_Diagnostics_Cleanup_Test extends WP_UnitTestCase {
	...
	/**
	 * Pending traces that have been idle past the 90-second threshold are
	 * promoted to `abandoned`; pending traces still inside the window are
	 * untouched.
	 *
	 * @return void
	 */
	public function test_pending_sweep_promotes_stale_pending_to_abandoned() {
		$this->store->create( 'stale-pending' );
		$this->store->create( 'fresh-pending' );
		$this->set_times( 'stale-pending', time() - 1000, time() - 200 );
		$this->set_times( 'fresh-pending', time() - 30, time() - 30 );

		$promoted = $this->cleanup->run_pending_sweep();

		$this->assertSame( 1, $promoted );
		$this->assertSame(
			WC_Stripe_Diagnostics_Trace_Store::STATUS_ABANDONED,
			$this->store->get( 'stale-pending' )['status']
		);
		$this->assertSame(
			WC_Stripe_Diagnostics_Trace_Store::STATUS_PENDING,
			$this->store->get( 'fresh-pending' )['status']
		);
	}

	/**
	 * The pending sweep only touches `pending` traces — `completed` traces
	 * past the idle threshold stay `completed`.
	 *
	 * @return void
	 */
	public function test_pending_sweep_leaves_completed_traces_alone() {
		$this->store->create( 'done' );
		$this->store->set_status( 'done', WC_Stripe_Diagnostics_Trace_Store::STATUS_COMPLETED );
		$this->set_times( 'done', time() - 1000, time() - 200 );
