...
class WC_Stripe_Payment_Method_Configurations_Test extends WC_Mock_Stripe_API_Unit_Test_Case {
	 ...
	 */
	public function test_get_payment_method_configuration_from_stripe( array $mock_pmc_response, ?string $expected_pmc_id, ?string $expected_fallback_pmc_id, ?string $initial_fallback_pmc_id = null ) {
		$mock_api = $this->getMockBuilder( \WC_Stripe_API::class )
			->disableOriginalConstructor()
			->getMock();
		...
		if ( null === $initial_fallback_pmc_id ) {
			...
			update_option( 'woocommerce_stripe_pmc_fallback_id', $initial_fallback_pmc_id );
		}
