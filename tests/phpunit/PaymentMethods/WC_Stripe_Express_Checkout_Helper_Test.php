...
class WC_Stripe_Express_Checkout_Helper_Test extends WP_UnitTestCase {
	 ...
	 * @param mixed  $virtual_taxable_product_filter_value Value for the filter `wc_stripe_show_express_checkout_taxable_virtual_products_with_geolocation`.
	 * @return void
	 *
	...
	public function test_hides_ece_if_cannot_compute_taxes(
		...
		$virtual_taxable_product_filter_value,
		$expected
	) {
		...
		$taxable_virtual_product_filter = null;
		if ( null !== $virtual_taxable_product_filter_value ) {
			$taxable_virtual_product_filter = function () use ( $virtual_taxable_product_filter_value ) {
				return $virtual_taxable_product_filter_value;
			};

			add_filter(
				'wc_stripe_show_express_checkout_taxable_virtual_products_with_geolocation',
				$taxable_virtual_product_filter
			);
		}
		...
		if ( null !== $taxable_virtual_product_filter ) {
			remove_filter( 'wc_stripe_show_express_checkout_taxable_virtual_products_with_geolocation', $taxable_virtual_product_filter );
		}
	...
	public function provide_test_hides_ece_if_cannot_compute_taxes() {
		...
		return [
			'Hide if cart has virtual product and tax is based on billing address.' => [
				...
				'virtual taxable product filter value' => null,
				'expected'                             => $hide,
			],
			'Hide if cart has virtual product, tax is based on billing address, and default customer location is set to geolocation.' => [
				'cart contents'                        => [ 'virtual_taxable', 'virtual_nontaxable' ],
				'is pay for order'                     => false,
				'tax based on'                         => 'billing',
				'filter value'                         => null,
			...
			'Hide if cart has virtual product, tax is based on billing address, default customer location is set to geolocation, and specific filter is null.' => [
				'cart contents'                        => [ 'virtual_taxable', 'virtual_nontaxable' ],
				'is pay for order'                     => false,
				'tax based on'                         => 'billing',
				'filter value'                         => null,
			...
			'Hide if cart has virtual product, tax is based on billing address, default customer location is set to geolocation, and specific filter is false.' => [
				'cart contents'                        => [ 'virtual_taxable', 'virtual_nontaxable' ],
				'is pay for order'                     => false,
				'tax based on'                         => 'billing',
				'filter value'                         => null,
				'default customer address'             => 'geolocation',
				'virtual taxable product filter value' => false,
				'expected'                             => $hide,
			],
			'Show if cart has virtual product, tax is based on billing address, default customer location is set to geolocation, and specific filter is false.' => [
				'cart contents'                        => [ 'virtual_taxable', 'virtual_nontaxable' ],
				'is pay for order'                     => false,
				'tax based on'                         => 'billing',
				'filter value'                         => null,
				'default customer address'             => 'geolocation',
				'virtual taxable product filter value' => true,
				'expected'                             => $show,
			],
			'Show if cart has virtual product and tax is based on billing address, but filter forces to show.' => [
				'cart contents'                        => [ 'virtual_taxable', 'virtual_nontaxable' ],
				'is pay for order'                     => false,
				'tax based on'                         => 'billing',
				'filter value'                         => false,
			...
			'Show if cart has virtual product and tax is based on shipping address.' => [
				'cart contents'                        => [ 'virtual_taxable', 'virtual_nontaxable' ],
				'is pay for order'                     => false,
			...
			'Show if taxes are not based on customer billing or shipping address.' => [
				'cart contents'                        => [ 'virtual_taxable' ],
				'is pay for order'                     => false,
				'tax based on'                         => 'base',
				'filter value'                         => null,
			...
			'Show if cart requires shipping.' => [
