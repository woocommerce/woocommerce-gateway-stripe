...
class WC_Stripe_Account_Test extends WP_UnitTestCase {
	...
	/**
	 * Test maybe_reconfigure_webhooks_after_adaptive_pricing_enabled (update_option_woocommerce_stripe_settings hook).
	 *
	 * @param array $old_value          Previous option value.
	 * @param array $new_value          New option value.
	 * @param bool  $should_reconfigure Whether maybe_reconfigure_webhooks_on_update is expected to be called.
	...
	public function test_maybe_reconfigure_webhooks_after_adaptive_pricing_enabled( $old_value, $new_value, $should_reconfigure ) {
		$this->account = $this->getMockBuilder( WC_Stripe_Account::class )
			...
			->getMock();
		$this->account->expects(
			$should_reconfigure ? $this->once() : $this->never()
		)->method( 'maybe_reconfigure_webhooks_on_update' );

		$this->account->maybe_reconfigure_webhooks_after_adaptive_pricing_enabled( $old_value, $new_value );
	}
}
