...
class PMC_Test_Helper {
	...
	public static function disable_pmc() {
		...
		unset( $stripe_settings['connection_type'], $stripe_settings['test_connection_type'] );
		WC_Stripe_Helper::update_main_stripe_settings( $stripe_settings );
	}
	 ...
	 * @return void
	 */
	public static function cache_mocked_configuration( $payment_methods = [] ) {
		...
		if ( empty( $payment_methods ) ) {
			$payment_method_configuration = array_merge(
				$payment_method_configuration,
				[
		...
		WC_Stripe_Database_Cache::set( WC_Stripe_Payment_Method_Configurations::CONFIGURATION_CACHE_KEY, (object) $payment_method_configuration );
	}
