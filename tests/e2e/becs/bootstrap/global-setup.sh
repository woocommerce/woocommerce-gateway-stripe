#!/usr/bin/env bash
#
# QIT globalSetup phase for BECS tests — runs inside the Docker container where WP-CLI is available.
#
# Derived from tests/e2e/qit/bootstrap/global-setup.sh with the following differences:
#   - Uses STRIPE_PUB_KEY_AU / STRIPE_SECRET_KEY_AU (Australian Stripe account)
#   - Sets store currency to AUD
#
...
echo "=== QIT Global Setup (BECS): Configuring WooCommerce store ==="
...
wp option set woocommerce_currency "AUD"
...
echo " - Configuring Stripe plugin settings (AU account)"
wp option update woocommerce_stripe_settings --format=json '{"enabled":"yes","title":"Credit Card (Stripe)","description":"Pay with your credit card via Stripe.","api_credentials":"","testmode":"yes","test_publishable_key":"'"${STRIPE_PUB_KEY_AU}"'","test_secret_key":"'"${STRIPE_SECRET_KEY_AU}"'","publishable_key":"","secret_key":"","webhook":"","test_webhook_secret":"","webhook_secret":"","inline_cc_form":"no","statement_descriptor":"","short_statement_descriptor":"","capture":"yes","payment_request":"yes","payment_request_button_type":"buy","payment_request_button_theme":"dark","payment_request_button_locations":["product","cart","checkout"],"payment_request_button_size":"default","saved_cards":"yes","logging":"no","upe_checkout_experience_enabled":"yes","test_connection_type":"connect"}'
...
echo "============================================================"
echo
echo "=== QIT Global Setup (BECS): Complete ==="
