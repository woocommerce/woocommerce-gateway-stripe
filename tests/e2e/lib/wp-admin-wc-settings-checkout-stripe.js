...
const TEST_PUBLISHABLE_KEY_SELECTOR = By.css(
	'#woocommerce_stripe_test_publishable_key'
);
const TEST_PUBLISHABLE_SECRET_KEY_SELECTOR = By.css(
	'#woocommerce_stripe_test_publishable_secret_key'
);
...
const STATEMENT_DESCRIPTOR_SELECTOR = By.css(
...
const PAYMENT_REQUEST_SELECTOR = By.css(
...
const ENABLE_BANCONTACT_SELECTOR = By.css(
	'#woocommerce_stripe_bancontact_enabled'
);
...
export default class WPAdminWCSettingsCheckoutStripe extends WPAdminWCSettings {
	...
	setTestPublishableSecretKey( testPublishableSecretKey ) {
		return helper.setWhenSettable(
			this.driver,
			TEST_PUBLISHABLE_SECRET_KEY_SELECTOR,
			testPublishableSecretKey
		);
	}
