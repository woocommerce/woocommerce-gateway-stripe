...
accepted_args=("--base_url" "--version" "--with_woo_setup" "--with_stripe_setup" "--project")
additional_args=""
for arg in "$@"; do
...
cross-env $test_env playwright test --config=tests/e2e/config/playwright.config.js $additional_args ${project:+--project=$project}
