...
const PaymentMethodUnavailableTaxConfigPill = ( { id, label } ) => {
	const unavailableReason = usePaymentMethodUnavailableReason( id );

	if (
		unavailableReason !==
		PAYMENT_METHOD_UNAVAILABLE_REASONS.UNSUPPORTED_TAX_CONFIGURATION
	) {
		return null;
	}
	...
	return (
		...
		>
			{ sprintf(
				...
				__(
					"%1$s is unavailable due to your store's tax configuration.",
					'woocommerce-gateway-stripe'
				),
				label
			) }
...
export default PaymentMethodUnavailableTaxConfigPill;
