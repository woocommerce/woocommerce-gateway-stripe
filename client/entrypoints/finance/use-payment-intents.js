...
import { PAYMENT_INTENTS_PATH } from './constants';
import apiFetch from '@wordpress/api-fetch';
import { addQueryArgs } from '@wordpress/url';
import { NAMESPACE } from 'wcstripe/data/constants';

/**
 * Fetches a page of Stripe payment intents.
 *
 ...
 * @param {?string} args.cursor  Payment intent id to start after, or null for the first page.
 * @return {{data: Array, hasMore: boolean, isLoading: boolean, error: ?string}} Fetch state.
 */
const usePaymentIntents = ( { perPage, cursor } ) => {
	...
	useEffect( () => {
		...
		apiFetch( {
			path: addQueryArgs( `${ NAMESPACE }${ PAYMENT_INTENTS_PATH }`, {
				limit: perPage,
