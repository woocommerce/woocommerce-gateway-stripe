...
- **Payment method instantiation registers hooks (CRITICAL — duplicate-charge incident):** never instantiate a `WC_Stripe_UPE_Payment_Method` subclass just to read metadata (title, label, capabilities). The constructor registers WP/WC hooks; instantiating a second time registers them twice. In 10.6.0 this caused subscription renewals to fire duplicate charges because `WC_Stripe_UPE_Payment_Method_OC` was instantiated to look up a display title. Use static methods on the payment method class, or read cached metadata, instead. Precedent: PR #5325 (10.6.0 → 10.6.1).
