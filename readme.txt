...
* Fix - Prevent classic checkout from submitting an empty payment method while the Stripe Payment Element is re-mounting after a checkout update (e.g. address change), which could fail the order and force a page refresh
* Fix - On Blocks checkout, wait for the Stripe Payment Element to finish re-mounting after a checkout update before submitting, instead of failing with an incomplete-payment error
