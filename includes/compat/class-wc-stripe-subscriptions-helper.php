...
class WC_Stripe_Subscriptions_Helper {
	...
	public static function is_subscription_payment_method_detached( $subscription ) {
		...
		$source_id = WC_Stripe_Subscription_Helper::get_instance()->get_stripe_source_id( $subscription );
		if ( ! $source_id ) {
			return false;
		}
	...
	public static function get_detached_payment_data_from_subscription( $subscription ) {
		return [
			...
			'customer_id'               => WC_Stripe_Subscription_Helper::get_instance()->get_stripe_customer_id( $subscription ),
