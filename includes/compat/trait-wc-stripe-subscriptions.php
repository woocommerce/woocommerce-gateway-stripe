...
trait WC_Stripe_Subscriptions_Trait {
	 ...
	 * @return void
	 */
	private function process_subscription_payment_attempt( $amount, $renewal_order, $retry = true, $previous_error = false, int $lock_expiry = 0, string &$acquired_lock = '', array &$keyed_requests = [] ) {
		$radar_reason = false;
		$response     = null;
				...
				if ( $this->is_retryable_error( $response->error ) && false === $radar_reason ) {
					...
					if ( $retry && ! $lock_expired ) {
						if ( 5 <= $this->retry_interval ) { // @phpstan-ignore-line (retry_interval is defined in classes using this class)
							$this->process_subscription_payment_attempt( $amount, $renewal_order, false, $response->error, $lock_expiry, $acquired_lock, $keyed_requests );
							return;
						}
						...
						$lock_expired = time() > $lock_expiry;
						if ( ! $lock_expired ) {
							$this->process_subscription_payment_attempt( $amount, $renewal_order, true, $response->error, $lock_expiry, $acquired_lock, $keyed_requests );
							return;
						}
					}
