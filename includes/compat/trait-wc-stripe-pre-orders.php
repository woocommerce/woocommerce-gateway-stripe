...
trait WC_Stripe_Pre_Orders_Trait {
	...
	public function maybe_init_pre_orders() {
		...
		$this->supports[] = 'pre-orders'; // @phpstan-ignore-line

		add_action( 'wc_pre_orders_process_pre_order_completion_payment_' . $this->id, [ $this, 'process_pre_order_release_payment' ] ); // @phpstan-ignore-line
		 ...
		 */
		if ( self::$has_attached_pre_order_integration_hooks || WC_Gateway_Stripe::ID !== $this->id ) { // @phpstan-ignore-line
			return;
		}
	...
	public function process_pre_order( $order_id ) {
		try {
			...
			$this->validate_minimum_order_amount( $order ); // @phpstan-ignore-line

			$prepared_source = $this->prepare_source( get_current_user_id(), true ); // @phpstan-ignore-line
			...
			$this->save_source_to_order( $order, $prepared_source ); // @phpstan-ignore-line

			// Try setting up a payment intent.
			// @phpstan-ignore-next-line
			$intent_secret = $this->setup_intent( $order, $prepared_source ); // @phpstan-ignore-line
	...
	public function process_pre_order_release_payment( $order, $retry = true ) {
		try {
			$source   = $this->prepare_order_source( $order ); // @phpstan-ignore-line
			$response = $this->create_and_confirm_intent_for_off_session( $order, $source ); // @phpstan-ignore-line

			$is_authentication_required = $this->is_authentication_required_for_payment( $response ); // @phpstan-ignore-line
			...
			if ( ! empty( $response->error ) && ! $is_authentication_required ) {
				...
				// Successful
				$this->process_response( $this->get_latest_charge_from_intent( $response ), $order ); // @phpstan-ignore-line
			}
		} catch ( Exception $e ) {
