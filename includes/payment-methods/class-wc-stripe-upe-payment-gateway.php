...
class WC_Stripe_UPE_Payment_Gateway extends WC_Stripe_Payment_Gateway {
	...
	/**
	 * Whether Optimized Checkout layout should be used for this request.
	 *
	 * When false, split Payment Element path is used.
	 *
	 * @return bool
	 */
	...
	private function process_payment_with_payment_method( int $order_id ) {
		...
		try {
			...
			if ( $this->oc_enabled && WC_Stripe_Helper::is_stripe_first_available_gateway( self::ID ) && isset( $payment_method_details->type ) ) {
	...
	public function set_payment_method_title_for_order( $order, $payment_method_type, $stripe_payment_method = false ) {
		...
		if ( $this->oc_enabled && WC_Stripe_Helper::is_stripe_first_available_gateway( self::ID ) && WC_Stripe_Payment_Methods::OC === $payment_method_type ) {
	...
	public function create_token_from_setup_intent( $setup_intent_id, $user ) {
		...
		try {
			...
			$payment_method = null;
			if ( $this->oc_enabled && WC_Stripe_Helper::is_stripe_first_available_gateway( self::ID ) ) {
	...
	protected function handle_saving_payment_method( WC_Order $order, $payment_method_object, string $payment_method_type ) {
		...
		} elseif ( $this->oc_enabled && WC_Stripe_Helper::is_stripe_first_available_gateway( self::ID ) && isset( $payment_method_object->type ) ) {
	...
	protected function get_redirect_url( $return_url, $payment_intent, $payment_information, $order, $payment_needed ) {
		$selected_payment_type = $this->oc_enabled && WC_Stripe_Helper::is_stripe_first_available_gateway( self::ID ) ? $payment_information['payment_method_details']->type : $payment_information['selected_payment_type'];
