...
class WC_Stripe_UPE_Payment_Method_Becs_Debit extends WC_Stripe_UPE_Payment_Method {
	...
	/**
	 * Creates a BECS payment token for the customer.
	 *
	...
	public function create_payment_token_for_user( $user_id, $payment_method ) {
		if ( ! isset( $payment_method->id ) || ! isset( $payment_method->au_becs_debit ) ) {
			return null;
		}
		...
		$payment_token->set_last4( $payment_method->au_becs_debit->last4 );
		$payment_token->set_bank_name( $payment_method->au_becs_debit->bank_name );
		$payment_token->set_account_type( $payment_method->au_becs_debit->account_type );
		$payment_token->set_fingerprint( $payment_method->au_becs_debit->fingerprint );
		$payment_token->save();
