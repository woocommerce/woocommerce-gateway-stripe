...
- **Filters** use the plugin's own `wc_stripe_agentic_commerce_*` prefix. Canonical names: `wc_stripe_agentic_commerce_should_sync_product`, `wc_stripe_agentic_commerce_disable_checkout`, `wc_stripe_agentic_commerce_addon_detection_meta_keys`, `wc_stripe_agentic_commerce_product_has_addon`. The `woocommerce_agentic_commerce_*` names for `should_sync_product` and `disable_checkout` shipped in 10.9.0 anticipating a shared cross-plugin contract that never materialized (WOOAI-636); they are deprecated via `apply_filters_deprecated` (seed the canonical default) — do not drop them.
