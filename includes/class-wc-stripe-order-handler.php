...
class WC_Stripe_Order_Handler extends WC_Stripe_Payment_Gateway {
	...
	public function prevent_cancelling_orders_awaiting_action( $cancel_order, $order ) {
		...
		if ( ! WC_Stripe_Order_Helper::is_stripe_gateway_order( $order ) || ! $this->get_intent_from_order( $order ) ) {
			return $cancel_order;
		}

		// If the order is awaiting action and was modified within the last day, don't cancel it.
		if ( WC_Stripe_Order_Helper::is_payment_awaiting_action( $order ) && $order->get_date_modified( 'edit' )->getTimestamp() > strtotime( '-1 day' ) ) {
			$cancel_order = false;
		}
