...
class WC_REST_Stripe_Account_Keys_Controller extends WC_Stripe_REST_Base_Controller {
	...
	public function configure_webhooks( WP_REST_Request $request ) {
		...
		$is_test_mode_enabled     = 'yes' === $settings['testmode'];
		$configure_opposite_mode = isset( $settings['testmode'] ) && ( $live_mode && $is_test_mode_enabled ) || ( ! $live_mode && ! $is_test_mode_enabled );
