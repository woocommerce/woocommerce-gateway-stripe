...
class WC_REST_Stripe_Locations_Controller extends WC_Stripe_REST_Base_Controller {
	...
	private function transform_pr_address( $location ) {
		if ( is_array( $location ) ) {
			...
			if ( ( isset( $location['country'] ) && WC_Stripe_Country_Code::PR === $location['country'] ) ||
				( isset( $location['state'] ) && WC_Stripe_Country_Code::PR === $location['state'] ) ) {
				$location['country'] = WC_Stripe_Country_Code::US;
				$location['state']   = WC_Stripe_Country_Code::PR;
			}
			return $location;
		}

		if ( ( isset( $location->address->country ) && WC_Stripe_Country_Code::PR === $location->address->country ) ||
			( isset( $location->address->state ) && WC_Stripe_Country_Code::PR === $location->address->state ) ) {
			$location->address->country = WC_Stripe_Country_Code::US;
			$location->address->state   = WC_Stripe_Country_Code::PR;
		}
		return $location;
	}
