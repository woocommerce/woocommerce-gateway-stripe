...
class WC_REST_Stripe_Agentic_Commerce_Controller extends WC_Stripe_REST_Base_Controller {
	...
	public function get_agentic_settings(): WP_REST_Response {
		...
		return rest_ensure_response(
			[
				...
				'auto_redirect_checkout_addons' => WC_Stripe_Agentic_Commerce_Integration::is_auto_redirect_checkout_addons_enabled(),
	...
	public function update_agentic_settings( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( $request->has_param( 'auto_redirect_checkout_addons' ) ) {
			$value = $request->get_param( 'auto_redirect_checkout_addons' ) ? 'yes' : 'no';
			update_option( WC_Stripe_Agentic_Commerce_Integration::AUTO_REDIRECT_CHECKOUT_ADDONS_OPTION, $value );
		}
