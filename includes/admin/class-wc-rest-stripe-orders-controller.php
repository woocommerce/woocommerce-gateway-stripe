...
class WC_REST_Stripe_Orders_Controller extends WC_Stripe_REST_Base_Controller {
	...
	public function capture_terminal_payment( $request ) {
		try {
			...
			if ( ! in_array( $intent->status, [ \Stripe\PaymentIntent::STATUS_PROCESSING, \Stripe\PaymentIntent::STATUS_REQUIRES_CAPTURE ], true ) ) {
			...
			if ( empty( $result ) || empty( $result->status ) || \Stripe\PaymentIntent::STATUS_SUCCEEDED !== $result->status ) {
				return new WP_Error(
