...
class WC_Stripe_Helper {
	...
	private static function should_load_scripts_for_ece_location( $location ) {
		...
		$is_ece_enabled   = self::get_settings( null, 'express_checkout' ) ?? 'yes';
		$button_locations = self::get_settings( null, 'express_checkout_button_locations' ) ?? [ 'product', 'cart' ];

		// The scripts should be loaded when all of the following are true:
		//   1. The ECE is enabled; and
		//   2. The ECE button location settings have an array value (saving an empty option in the GUI results in non-array value); and
		//   3. The ECE is enabled at $location.
		return 'yes' === $is_ece_enabled && is_array( $button_locations ) && in_array( $location, $button_locations, true );
	}
