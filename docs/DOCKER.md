...
- From the root of this project, run `pnpm run up`
...
- Use `pnpm run down` to stop the running containers
- The state of the environment will be persisted in `docker/wordpress` and `docker/data`. To restart the environment simply run `pnpm run up` again. To start afresh, delete these folders and let `pnpm run up` re-create them.
